...
export function MemberEditModal({ businessId, actorRole, actorFullControl, actorIsFounder, member, open, onClose }: { businessId: string | number; actorRole: BusinessRole; actorFullControl: boolean; actorIsFounder: boolean; member: Member | null; open: boolean; onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      if (!member) return;
      const { full_control, ownership_percent, profit_share_percent, salary_amount, salary_visible_to_staff, ...rest } = form;
      // Only send the ownership fields if they actually changed from the member's
      // current schedule — otherwise every unrelated edit (e.g. just the job title)
      // would create a fresh, identical dated ownership record.
      const ownershipChanged = profitBased && (Number(ownership_percent || 0) !== member.ownership_percent || Number(profit_share_percent || 0) !== member.profit_share_percent);
    ...
    <Modal open={open} onClose={onClose} title={`Edit ${member.name}`} description={editingFounder ? "This person created the business, so they always keep full control." : "Update role, title, pay and access for this team member."} footer={<><Button variant="ghost" onClick={onClose}>Cancel</Button><Button type="submit" form="member-edit-form" loading={mutation.isPending}>Save changes</Button></>} size="lg">
      <form id="member-edit-form" onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
        ...
        {profitBased ? (
          ...
          <>
            <FieldShell label="Monthly salary amount" htmlFor="edit-member-salary-amount" error={errors.salary_amount?.[0]} hint="Optional"><Input id="edit-member-salary-amount" type="number" min="0" step="0.01" placeholder="0.00" value={form.salary_amount} onChange={(event) => update("salary_amount", event.target.value)} /></FieldShell>
