...
const blank = { name: "", email: "", phone: "", password: "", role: "employee" as BusinessRole, full_control: false, title: "", joined_at: today(), pay_type: "fixed_salary" as PayType, salary_amount: "0", salary_visible_to_staff: false, ownership_percent: "0", profit_share_percent: "0" };
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add team member" description="Use an existing account email or set a temporary password for a new user." footer={<><Button variant="ghost" onClick={onClose}>Cancel</Button><Button type="submit" form="member-form" loading={mutation.isPending}>Add member</Button></>} size="lg">
      <form id="member-form" onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }} className="grid gap-4 sm:grid-cols-2">
        ...
        <FieldShell label="Joined on" htmlFor="member-joined-at" error={errors.joined_at?.[0]} hint="When they actually started — backdate this if you're registering them late, so accrued salary counts from their real start date"><Input id="member-joined-at" type="date" max={today()} value={form.joined_at} onChange={(event) => update("joined_at", event.target.value)} /></FieldShell>
