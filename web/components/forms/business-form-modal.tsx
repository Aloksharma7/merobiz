    ...
    >
      <form id="business-form" onSubmit={submit} className="space-y-5">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <FieldShell label="Your ownership" htmlFor="ownership" error={errors.ownership_percent?.[0]} hint="Percentage" required><Input id="ownership" type="number" min="0" max="100" step="0.0001" value={form.ownership_percent} onChange={(event) => { update("ownership_percent", event.target.value); if (form.profit_share_percent === form.ownership_percent) update("profit_share_percent", event.target.value); }} required /></FieldShell>
          <FieldShell label="Your profit share" htmlFor="profit-share" error={errors.profit_share_percent?.[0]} hint="Usually same as ownership"><Input id="profit-share" type="number" min="0" max="100" step="0.0001" value={form.profit_share_percent} onChange={(event) => update("profit_share_percent", event.target.value)} /></FieldShell>
        </div>
        ...
        {advanced ? (
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <FieldShell label="PAN number" htmlFor="pan" error={errors.pan_number?.[0]}><Input id="pan" value={form.pan_number} onChange={(event) => update("pan_number", event.target.value)} /></FieldShell>
            <FieldShell label="VAT number" htmlFor="vat" error={errors.vat_number?.[0]}><Input id="vat" value={form.vat_number} onChange={(event) => update("vat_number", event.target.value)} /></FieldShell>
            <FieldShell label="Default tax rate" htmlFor="tax" error={errors.default_tax_rate?.[0]} hint="Percentage"><Input id="tax" type="number" min="0" max="100" step="0.01" value={form.default_tax_rate} onChange={(event) => update("default_tax_rate", event.target.value)} /></FieldShell>
            <FieldShell label="Phone" htmlFor="business-phone" error={errors.phone?.[0]}><Input id="business-phone" value={form.phone} onChange={(event) => update("phone", event.target.value)} /></FieldShell>
            <FieldShell label="Billing email" htmlFor="business-email" error={errors.email?.[0]} className="sm:col-span-2"><Input id="business-email" type="email" value={form.email} onChange={(event) => update("email", event.target.value)} /></FieldShell>
            <FieldShell label="Address" htmlFor="business-address" error={errors.address?.[0]} className="sm:col-span-2"><Textarea id="business-address" value={form.address} onChange={(event) => update("address", event.target.value)} /></FieldShell>
          </div>
        ) : null}
      </form>
    </Modal>
