...
export function TrendChart({ data, currency = "NPR", portfolio = false }: { data: TrendPoint[]; currency?: string; portfolio?: boolean }) {
  return (
    <Card className="min-w-0 overflow-hidden">
      <CardHeader title="Performance trend" description="Six-month view of sales and profit" action={<div className="flex items-center gap-3 text-[11px] font-semibold text-[var(--ink-soft)]"><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-[var(--brand)]" />Net sales</span><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-[var(--accent)]" />{portfolio ? "Your profit" : "Net profit"}</span></div>} />
      <CardBody className="h-[320px] px-2 pb-3 pt-5 sm:px-5">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 8, right: 10, left: 0, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey={portfolio ? "attributable_profit" : "net_profit"} stroke="#eaa737" strokeWidth={2.5} fill="url(#profitGradient)" dot={false} activeDot={{ r: 4, strokeWidth: 2, fill: "white", stroke: "#eaa737" }} />
          </AreaChart>
        </ResponsiveContainer>
      </CardBody>
    </Card>
