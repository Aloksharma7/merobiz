...
import { Banknote, Check, Clock3, Download, Pencil, Plus, Receipt, Search, Trash2, WalletCards, X } from "lucide-react";
    ...
    <div className="space-y-7">
      ...
      <Card className="overflow-hidden">
        ...
        {query.isLoading ? <TableLoading /> : rows.length ? (
          <>
            <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[850px] text-left text-sm"><thead><tr className="bg-[var(--surface-soft)] text-[10px] uppercase tracking-[0.11em] text-[var(--ink-soft)]"><th className="px-5 py-3 font-bold">Date & category</th><th className="px-4 py-3 font-bold">Vendor</th><th className="px-4 py-3 font-bold">Method</th><th className="px-4 py-3 font-bold">Status</th><th className="px-4 py-3 text-right font-bold">Amount</th><th className="px-5 py-3 text-right font-bold">Actions</th></tr></thead><tbody className="divide-y divide-[var(--line)]">{rows.map((expense) => <tr key={expense.id} className="hover:bg-[var(--surface-soft)]"><td className="px-5 py-3.5"><p className="font-bold">{expense.category}</p><p className="mt-0.5 text-xs text-[var(--ink-soft)]">{prettyDate(expense.expense_date)}{expense.reference ? ` · ${expense.reference}` : ""}{!expense.affects_profit ? " · cost of goods/service already priced into a sale" : ""}</p></td><td className="px-4 py-3.5 font-medium">{expense.vendor || "—"}</td><td className="px-4 py-3.5 text-[var(--ink-soft)]">{humanize(expense.payment_method)}</td><td className="px-4 py-3.5"><Badge tone={statusTone(expense.status)}>{expense.status}</Badge></td><td className="px-4 py-3.5 text-right font-black">{money(expense.amount, business.currency)}</td><td className="px-5 py-3.5"><div className="flex justify-end gap-1">{canApprove && expense.status === "pending" ? <><Button size="sm" variant="quiet" onClick={() => statusMutation.mutate({ id: expense.id, next: "approved" })}>Approve</Button><Button size="sm" variant="ghost" onClick={() => statusMutation.mutate({ id: expense.id, next: "rejected" })}>Reject</Button></> : null}{canEditExpense(expense) ? <button type="button" onClick={() => setEditingExpense(expense)} className="grid h-9 w-9 place-items-center rounded-lg text-[var(--ink-soft)] hover:bg-[var(--surface-soft)]" aria-label="Edit expense"><Pencil size={15} /></button> : null}{canDeleteExpense(expense) ? <button type="button" onClick={() => { if (window.confirm("Delete this expense record?")) deleteMutation.mutate(expense.id); }} className="grid h-9 w-9 place-items-center rounded-lg text-[var(--ink-soft)] hover:bg-[var(--danger-soft)] hover:text-[var(--danger)]" aria-label="Delete expense"><Trash2 size={15} /></button> : null}</div></td></tr>)}</tbody></table></div>
            <div className="divide-y divide-[var(--line)] md:hidden">{rows.map((expense) => <div key={expense.id} className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold">{expense.category}</p><p className="mt-1 text-xs text-[var(--ink-soft)]">{prettyDate(expense.expense_date)} · {expense.vendor || "No vendor"}</p></div><Badge tone={statusTone(expense.status)}>{expense.status}</Badge></div><div className="mt-3 flex items-center justify-between"><p className="text-lg font-black">{money(expense.amount, business.currency)}</p><div className="flex gap-2">{canApprove && expense.status === "pending" ? <><button type="button" onClick={() => statusMutation.mutate({ id: expense.id, next: "approved" })} className="grid h-11 w-11 place-items-center rounded-lg bg-[var(--brand-soft)] text-[var(--brand)]" aria-label="Approve"><Check size={16} /></button><button type="button" onClick={() => statusMutation.mutate({ id: expense.id, next: "rejected" })} className="grid h-11 w-11 place-items-center rounded-lg bg-[var(--danger-soft)] text-[var(--danger)]" aria-label="Reject"><X size={16} /></button></> : null}{canEditExpense(expense) ? <button type="button" onClick={() => setEditingExpense(expense)} className="grid h-11 w-11 place-items-center rounded-lg border border-[var(--line)] text-[var(--ink-soft)] hover:bg-[var(--surface-soft)]" aria-label="Edit expense"><Pencil size={15} /></button> : null}{canDeleteExpense(expense) ? <button type="button" onClick={() => { if (window.confirm("Delete this expense record?")) deleteMutation.mutate(expense.id); }} className="grid h-11 w-11 place-items-center rounded-lg border border-[var(--line)] text-[var(--ink-soft)] hover:bg-[var(--danger-soft)] hover:text-[var(--danger)]" aria-label="Delete expense"><Trash2 size={15} /></button> : null}</div></div></div>)}</div>
      ...
      <ExpenseFormModal businessId={businessId} open={!!editingExpense} onClose={() => setEditingExpense(null)} expense={editingExpense} isInstallment={business.is_installment} />
    </div>
  );
}
