...
function CatalogPageContent() {
  ...
  const canSeeFinancials = can(business, "dashboard.financial");
    ...
    <div className="space-y-7">
      ...
      <Card className="overflow-hidden">
        ...
        {query.isLoading ? <TableLoading /> : rows.length ? <>
          <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[850px] text-left text-sm"><thead><tr className="bg-[var(--surface-soft)] text-[10px] uppercase tracking-[0.11em] text-[var(--ink-soft)]"><th className="px-5 py-3 font-bold">Item</th><th className="px-4 py-3 font-bold">Type</th><th className="px-4 py-3 text-right font-bold">Selling price</th>{canSeeFinancials ? <><th className="px-4 py-3 text-right font-bold">Direct cost</th><th className="px-4 py-3 text-right font-bold">Margin</th></> : null}<th className="px-4 py-3 font-bold">Stock</th><th className="px-5 py-3 text-right font-bold">Actions</th></tr></thead><tbody className="divide-y divide-[var(--line)]">{rows.map((product) => { const margin = canSeeFinancials && product.cost_price !== null && product.sale_price ? ((product.sale_price - product.cost_price) / product.sale_price) * 100 : 0; return <tr key={product.id} className="hover:bg-[var(--surface-soft)]"><td className="px-5 py-3.5"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--brand-soft)] text-[var(--brand)]"><PackageCheck size={18} /></span><div><p className="font-bold">{product.name}</p><p className="mt-0.5 text-xs text-[var(--ink-soft)]">{product.sku || "No SKU"} · per {product.unit}</p></div></div></td><td className="px-4 py-3.5"><Badge tone={product.type === "digital_subscription" ? "info" : product.type === "product" ? "warning" : "success"}>{humanize(product.type)}</Badge></td><td className="px-4 py-3.5 text-right font-black">{money(product.sale_price, business.currency)}</td>{canSeeFinancials ? <><td className="px-4 py-3.5 text-right text-[var(--ink-soft)]">{product.cost_price !== null ? money(product.cost_price, business.currency) : "—"}</td><td className="px-4 py-3.5 text-right font-bold">{number(margin, 1)}%</td></> : null}<td className="px-4 py-3.5">{product.track_inventory ? <div className="flex items-center gap-2"><span className={product.stock_quantity <= product.reorder_level ? "font-bold text-[var(--danger)]" : "font-semibold"}>{number(product.stock_quantity, product.stock_quantity % 1 ? 2 : 0)} {product.unit}</span>{product.stock_quantity <= product.reorder_level ? <Badge tone="danger">Low stock</Badge> : null}</div> : <span className="text-[var(--ink-soft)]">Not tracked</span>}</td><td className="px-5 py-3.5"><div className="flex justify-end gap-1">{canManage ? <><button type="button" onClick={() => setEditing(product)} className="grid h-9 w-9 place-items-center rounded-lg text-[var(--ink-soft)] hover:bg-[var(--brand-soft)] hover:text-[var(--brand)]" aria-label="Edit item"><Edit3 size={15} /></button><button type="button" onClick={() => { if (window.confirm("Archive this item? Past invoices remain unchanged.")) archive.mutate(product.id); }} className="grid h-9 w-9 place-items-center rounded-lg text-[var(--ink-soft)] hover:bg-[var(--danger-soft)] hover:text-[var(--danger)]" aria-label="Archive item"><Trash2 size={15} /></button></> : null}</div></td></tr>; })}</tbody></table></div>
          <div className="grid gap-3 p-4 sm:grid-cols-2 md:hidden">{rows.map((product) => <button type="button" key={product.id} disabled={!canManage} onClick={() => setEditing(product)} className="rounded-2xl border border-[var(--line)] bg-white p-4 text-left enabled:transition enabled:hover:border-[#b3c3b6] enabled:hover:bg-[var(--surface-soft)]"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-bold">{product.name}</p><p className="mt-1 text-xs text-[var(--ink-soft)]">{product.sku || humanize(product.type)}</p></div><Badge tone={product.active ? "success" : "neutral"}>{product.active ? "Active" : "Inactive"}</Badge></div><div className="mt-4 flex items-end justify-between gap-3">{canSeeFinancials ? <div><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--ink-soft)]">Direct cost</p><p className="mt-1 text-sm font-semibold">{product.cost_price !== null ? money(product.cost_price, business.currency) : "—"}</p></div> : <span />}<p className="text-lg font-black">{money(product.sale_price, business.currency)}</p></div></button>)}</div>
