...
import { ArrowRight, Boxes, CircleDollarSign, ContactRound, HandCoins, Landmark, Plus, ReceiptText, TrendingUp, WalletCards } from "lucide-react";
...
export default function BusinessDashboardPage() {
  ...
  const branding = brandingFor(data.business);
  ...
  return (
    <div className="space-y-7">
      <PageHeader
        eyebrow={employee ? <span className="flex items-center gap-2"><BusinessMark business={data.business} compact />{branding.tagline || "My sales"}</span> : <span className="flex items-center gap-2"><span className="grid h-6 min-w-6 place-items-center rounded-md bg-[var(--brand-soft)] px-1.5 text-[10px] font-black text-[var(--brand-deep)]">{data.business.code}</span>{humanize(data.business.business_type)}</span>}
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard emphasis={financial} label={owner ? "Your estimated profit" : admin ? "Business net profit" : "My sales"} value={owner ? data.summary.attributable_profit ?? 0 : admin ? data.summary.net_profit : data.summary.net_sales} currency={currency} icon={CircleDollarSign} hint={owner ? `${data.business.profit_share_percent}% of live net profit` : admin ? "Sales after costs and expenses" : "Only invoices created by you"} />
      ...
      {financial ? (
        <section className="grid gap-3 rounded-[var(--radius)] border border-[var(--line)] bg-white p-3 shadow-[var(--shadow-sm)] sm:grid-cols-2 xl:grid-cols-4">
          {[
            ["Gross profit", data.summary.gross_profit, "Sales minus direct cost"],
            ["Approved expenses", data.summary.expenses, "Operating costs"],
            ["Staff commissions", data.summary.commissions, "Generated from sales"],
            ["Business net profit", data.summary.net_profit, "Before owner distribution"],
      ...
      <section className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1.65fr)_minmax(330px,0.85fr)]">
        <TrendChart data={data.trend} currency={currency} salesOnly={!financial} />
