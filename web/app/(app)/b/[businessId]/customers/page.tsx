    ...
    <div className="space-y-7">
      ...
      <Card className="overflow-hidden">
        ...
        {query.isLoading ? <TableLoading /> : rows.length ? <>
          <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[760px] text-left text-sm"><thead><tr className="bg-[var(--surface-soft)] text-[10px] uppercase tracking-[0.11em] text-[var(--ink-soft)]"><th className="px-5 py-3 font-bold">Customer</th><th className="px-4 py-3 font-bold">Contact</th><th className="px-4 py-3 font-bold">PAN</th><th className="px-4 py-3 text-right font-bold">Outstanding</th><th className="px-5 py-3 text-right font-bold">Actions</th></tr></thead><tbody className="divide-y divide-[var(--line)]">{rows.map((customer) => <tr key={customer.id} className="cursor-pointer hover:bg-[var(--surface-soft)]" onClick={() => router.push(`/b/${businessId}/customers/${customer.id}`)}><td className="px-5 py-3.5"><Link href={`/b/${businessId}/customers/${customer.id}`} onClick={(event) => event.stopPropagation()} className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--brand-soft)] text-sm font-black text-[var(--brand)]">{customer.name.slice(0, 2).toUpperCase()}</span><div><p className="font-bold hover:text-[var(--brand)]">{customer.name}</p><p className="mt-0.5 max-w-[240px] truncate text-xs text-[var(--ink-soft)]">{customer.address || "No address"}</p></div></Link></td><td className="px-4 py-3.5"><p className="text-sm">{customer.phone || "—"}</p><p className="mt-0.5 text-xs text-[var(--ink-soft)]">{customer.email || "No email"}</p></td><td className="px-4 py-3.5 text-[var(--ink-soft)]">{customer.pan_number || "—"}</td><td className="px-4 py-3.5 text-right"><div className="flex items-center justify-end gap-2"><Badge tone={customer.outstanding_balance > 0 ? "warning" : "success"}>{customer.outstanding_balance > 0 ? "Balance due" : "Clear"}</Badge><span className={customer.outstanding_balance > 0 ? "font-black text-[var(--danger)]" : "font-bold"}>{money(customer.outstanding_balance, business.currency)}</span></div></td><td className="px-5 py-3.5" onClick={(event) => event.stopPropagation()}><div className="flex justify-end gap-1">{canManage ? <><button type="button" onClick={() => setEditing(customer)} className="grid h-9 w-9 place-items-center rounded-lg text-[var(--ink-soft)] hover:bg-[var(--brand-soft)] hover:text-[var(--brand)]" aria-label="Edit customer"><Edit3 size={15} /></button><button type="button" onClick={() => { if (window.confirm("Archive this customer? Existing invoices remain unchanged.")) archive.mutate(customer.id); }} className="grid h-9 w-9 place-items-center rounded-lg text-[var(--ink-soft)] hover:bg-[var(--danger-soft)] hover:text-[var(--danger)]" aria-label="Archive customer"><Trash2 size={15} /></button></> : null}</div></td></tr>)}</tbody></table></div>
          <div className="divide-y divide-[var(--line)] md:hidden">{rows.map((customer) => <Link href={`/b/${businessId}/customers/${customer.id}`} key={customer.id} className="block w-full p-4 text-left hover:bg-[var(--surface-soft)]"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-bold">{customer.name}</p><p className="mt-1 flex items-center gap-1.5 text-xs text-[var(--ink-soft)]"><Phone size={12} />{customer.phone || "No phone"}</p><p className="mt-1 flex items-center gap-1.5 truncate text-xs text-[var(--ink-soft)]"><Mail size={12} />{customer.email || "No email"}</p></div><Badge tone={customer.outstanding_balance > 0 ? "warning" : "success"}>{customer.outstanding_balance > 0 ? "Balance due" : "Clear"}</Badge></div><p className="mt-3 text-lg font-black">{money(customer.outstanding_balance, business.currency)}</p></Link>)}</div>
