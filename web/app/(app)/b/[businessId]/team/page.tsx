    ...
    <div className="space-y-7">
      ...
      <Card className="overflow-hidden">
        {query.isLoading ? <TableLoading /> : rows.length ? <>
          ...
          <div className="grid gap-3 p-4 sm:grid-cols-2 md:hidden">{[...rows].sort((a, b) => b.sales - a.sales).map((member) => <button type="button" disabled={!canManage} onClick={() => setEditing(member)} key={member.id} className="rounded-2xl border border-[var(--line)] p-4 text-left enabled:transition enabled:hover:border-[#b3c3b6] enabled:hover:bg-[var(--surface-soft)]"><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--brand-soft)] text-xs font-black text-[var(--brand)]">{member.initials}</span><div className="min-w-0 flex-1"><p className="truncate font-bold">{member.name}</p><p className="mt-0.5 truncate text-xs text-[var(--ink-soft)]">{roleLabel(member)} · {member.invoice_count} invoices</p></div><Badge tone={member.active ? "success" : "neutral"}>{member.active ? "Active" : "Off"}</Badge></div><div className="mt-4 flex items-end justify-between"><p className="text-xs text-[var(--ink-soft)]">{member.salary_pending < 0 ? "Overpaid" : member.pay_type === "fixed_salary" ? "Salary owed" : "Commission owed"}<br /><strong className={member.salary_pending > 0 ? "text-[var(--danger)]" : member.salary_pending < 0 ? "text-[var(--info)]" : "text-[var(--ink)]"}>{money(Math.abs(member.salary_pending), business.currency)}</strong>{member.outstanding_loan > 0 ? <span className="block text-[11px] font-semibold text-[var(--accent)]">Loan {money(member.outstanding_loan, business.currency)}</span> : null}</p><p className="text-lg font-black">{money(member.sales, business.currency)}</p></div></button>)}</div>
